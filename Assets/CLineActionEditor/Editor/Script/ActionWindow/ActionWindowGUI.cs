...
namespace SuperCLine.ActionEngine.Editor
{
    ...
    internal sealed partial class ActionWindow
    {
        ...
        [SerializeReference] private ActorSplitter propertySplitter = null;
        [SerializeReference] private ActorSplitter inspectorSplitter = null;
        ...
        private void InitGUI()
        {
            ...
            if (actorSplitter == null)
            {
                ...
                actorSplitter.Init((width) => { headerWidth = width; }, () => { return headerWidth; });

                propertySplitter = ScriptableObject.CreateInstance<ActorSplitter>();
                propertySplitter.Init((width) => { propertyWidth = inspectorWidth - (rectWindow.width - width) - 2; },
                    () => { return rectWindow.width - inspectorWidth + propertyWidth + 2; });

                inspectorSplitter = ScriptableObject.CreateInstance<ActorSplitter>();
                inspectorSplitter.Init((width) => { inspectorWidth = rectWindow.width - width; }, 
                    () => { return rectWindow.width - inspectorWidth; });
