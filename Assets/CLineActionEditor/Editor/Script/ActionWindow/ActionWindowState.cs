...
namespace SuperCLine.ActionEngine.Editor
{
    ...
    internal sealed partial class ActionWindow
    {
        ...
        public void HandlePreManipulator()
        {
            ...
            propertySplitter.HandleManipulatorsEvents(this, Event.current);
            inspectorSplitter.HandleManipulatorsEvents(this, Event.current);
                    ...
                    if (e.eventProperty != null && e.eventProperty.EventData != null && e.eventProperty.EventData is EventPlayAnim epa)
                    {
                        menu.AddItem(editorResources.contextResetTotalTime, false, () =>
                        {
                            if (actorTreeViewAction != null)
                            {
                                ...
                                length = ToSecond(e.eventProperty.TriggerTime) + state.motion.averageDuration;
                            }
                        });
                    }
                    else
                    {
                        menu.AddItem(editorResources.contextDuplicateEvent, false, () =>
                        {
                            var newAE = e.Clone();
                            newAE.start += snapInterval;
                        });
                        menu.AddItem(editorResources.contextCopyEventValue, false, () =>
                        {
                            copyEvent = e;
                        });
                        menu.AddItem(editorResources.contextPasteEventValue, false, () =>
                        {
                            if (copyEvent != null)
                            {
