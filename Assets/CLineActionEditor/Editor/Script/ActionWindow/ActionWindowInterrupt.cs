...
namespace SuperCLine.ActionEngine.Editor
{
    ...
    internal sealed partial class ActionWindow
    {
        ...
        public void DrawActionInterrupt(ActionInterrupt interrupt, bool template)
        {
            ...
            if (template)
            {
                using (new GUIColorScope(editorResources.colorRefreshInterrupt))
                {
                    if (GUILayout.Button(editorResources.refreshInterrupt))
                    {
                        RefreshInterrupt(interrupt);
                    }
                }
                GUILayout.Space(2);
            }
        ...
        private void SaveActionInterrupt()
        {
            ...
            string path = string.Format("{0}ActionInterrupt/{1}.json", ResourceMgr.gamedataPath, UnitWrapper.Instance.ActionGroupName);
        ...
        private void DeserializeInterrupt()
        {
            ...
            StringBuilder sb = new StringBuilder();
            sb.AppendFormat("{0}ActionInterrupt/{1}.json", ResourceMgr.gamedataPath, groupName);
                ...
                for (int i = 0; i < interrupts.Count; ++i)
                {
                    ...
                    using (var itr = actorActionTree.children.GetEnumerator())
                    {
                        while (itr.MoveNext())
                        {
                            ...
                            if ((ac.ID == null && aci.ActionID == null) || ac.ID.Equals(aci.ActionID))
                            {
        ...
        private void RefreshInterrupt(ActionInterrupt interrupt)
        {
            using (var itr = actorActionTree.children.GetEnumerator())
            {
                while (itr.MoveNext())
                {
                    var ap = itr.Current as ActorProperty;
                    var ac = ap.property as Action;
                    using (var itrInterrupt = ac.InterruptList.GetEnumerator())
                    {
                        while (itrInterrupt.MoveNext())
                        {
                            var data = itrInterrupt.Current.EventData as ActionInterrupt;
                            if (data.ActionID == interrupt.ActionID)
                            {
                                data.CopyFrom(interrupt);
                            }
                        }
                    }
                }
            ...
            EditorUtility.DisplayDialog("INFO", "Succeed to refresh!", "OK");
        }
