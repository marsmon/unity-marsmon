...
namespace SuperCLine.ActionEngine.Editor
{
    ...
    internal sealed partial class ActionWindow
    {
                ...
                if (!searcher.HasSearchbarFocused())
                {
                    GUILayout.BeginHorizontal(editorResources.btnToolBoxStyle, GUILayout.Width(rectInspectorLeft.width), GUILayout.Height(16));
                    {
                        if (GUILayout.Button("New"))
                        {
                            ...
                            ac.TotalTime = 1666;
                            ac.ID = ActionEngine.Helper.NonceStr(25);

                            ActionInterrupt interrupt = new ActionInterrupt();
                            interrupt.ActionID = ac.ID;
        ...
        private void SaveAction()
        {
            ...
            string path = string.Format("{0}Action/{1}.json", ResourceMgr.gamedataPath, UnitWrapper.Instance.ActionGroupName);
        ...
        private void OnInputChanged(string searchString)
        {
            ...
            {
                foreach (var item in actorActionTree.children)
                {
                    var ap = item as ActorProperty;
                    var ac = ap.property as Action;
                    if (!string.IsNullOrEmpty(ac.ID) && ac.ID.Contains(searchString))
                    {
                        searcher.AddResult(ac.ID);
                    }
                }
            }
        }
