...
namespace com.tencent.pandora
{
	public static class ToLuaExport
	{
		...
		{
			ToLuaExport.<Generate>c__AnonStorey97 <Generate>c__AnonStorey = new ToLuaExport.<Generate>c__AnonStorey97();
		...
		{
			ToLuaExport.<GenConstruct>c__AnonStorey9A <GenConstruct>c__AnonStorey9A = new ToLuaExport.<GenConstruct>c__AnonStorey9A();
			...
			<GenConstruct>c__AnonStorey9A.list = new List<ConstructorInfo>();
			for (int k = 0; k < constructors.Length; k++)
			{
				if (!ToLuaExport.HasDecimal(constructors[k].GetParameters()))
				{
					if (!ToLuaExport.IsObsolete(constructors[k]))
					{
						...
						int num = <GenConstruct>c__AnonStorey9A.list.FindIndex((ConstructorInfo p) => ToLuaExport.CompareMethod(p, r) >= 0);
						if (num >= 0)
						{
							if (ToLuaExport.CompareMethod(<GenConstruct>c__AnonStorey9A.list.get_Item(num), r) == 2)
							{
								<GenConstruct>c__AnonStorey9A.list.RemoveAt(num);
								<GenConstruct>c__AnonStorey9A.list.Add(r);
							}
						}
						else
						{
			...
			if (<GenConstruct>c__AnonStorey9A.list.get_Count() == 0)
			{
				if (!ToLuaExport.type.get_IsValueType())
				{
					ToLuaExport.NoConsturct();
			...
			<GenConstruct>c__AnonStorey9A.list.Sort(new Comparison<ConstructorInfo>(ToLuaExport.Compare));
			ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
			ToLuaExport.sb.AppendFormat("\tstatic int _Create{0}(IntPtr L)\r\n", ToLuaExport.wrapClassName);
			ToLuaExport.sb.AppendLine("\t{");
			ToLuaExport.sb.AppendLine("\t\tint count = LuaDLL.lua_gettop(L);");
			...
			int i;
			for (i = 0; i < <GenConstruct>c__AnonStorey9A.list.get_Count(); i++)
			{
				int num2 = <GenConstruct>c__AnonStorey9A.list.FindIndex((ConstructorInfo p) => p != <GenConstruct>c__AnonStorey9A.list.get_Item(i) && p.GetParameters().Length == <GenConstruct>c__AnonStorey9A.list.get_Item(i).GetParameters().Length);
				if (num2 >= 0 || (ToLuaExport.HasOptionalParam(<GenConstruct>c__AnonStorey9A.list.get_Item(i).GetParameters()) && <GenConstruct>c__AnonStorey9A.list.get_Item(i).GetParameters().Length > 1))
				{
					list.Add(<GenConstruct>c__AnonStorey9A.list.get_Item(i));
				}
			}
			MethodBase methodBase = <GenConstruct>c__AnonStorey9A.list.get_Item(0);
			bool flag = <GenConstruct>c__AnonStorey9A.list.get_Item(0).GetParameters().Length == 0;
			...
			else
			{
				...
				if (<GenConstruct>c__AnonStorey9A.list.get_Count() == 1 || methodBase.GetParameters().Length != <GenConstruct>c__AnonStorey9A.list.get_Item(1).GetParameters().Length)
				{
			...
			ToLuaExport.sb.AppendLine("\t\t{");
			int num3 = ToLuaExport.ProcessParams(methodBase, 3, true, <GenConstruct>c__AnonStorey9A.list.get_Count() > 1, false);
			ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num3);
			ToLuaExport.sb.AppendLine("\t\t}");
			for (int j = 1; j < <GenConstruct>c__AnonStorey9A.list.get_Count(); j++)
			{
				flag = (<GenConstruct>c__AnonStorey9A.list.get_Item(j).GetParameters().Length == 0 || flag);
				methodBase = <GenConstruct>c__AnonStorey9A.list.get_Item(j);
				ParameterInfo[] parameters3 = methodBase.GetParameters();
				if (!ToLuaExport.HasOptionalParam(methodBase.GetParameters()))
				{
					if (list.Contains(<GenConstruct>c__AnonStorey9A.list.get_Item(j)))
					{
		...
		{
			ToLuaExport.<GenOverrideFunc>c__AnonStorey9D <GenOverrideFunc>c__AnonStorey9D = new ToLuaExport.<GenOverrideFunc>c__AnonStorey9D();
			<GenOverrideFunc>c__AnonStorey9D.list = new List<MethodInfo>();
			for (int k = 0; k < ToLuaExport.methods.Length; k++)
			{
				...
				{
					ToLuaExport.Push(<GenOverrideFunc>c__AnonStorey9D.list, ToLuaExport.methods[k]);
				}
			}
			if (<GenOverrideFunc>c__AnonStorey9D.list.get_Count() == 1)
			{
				return <GenOverrideFunc>c__AnonStorey9D.list.get_Item(0);
			}
			<GenOverrideFunc>c__AnonStorey9D.list.Sort(new Comparison<MethodInfo>(ToLuaExport.Compare));
			ToLuaExport.sb.AppendLine("\r\n\t[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]");
			...
			List<MethodInfo> list = new List<MethodInfo>();
			int i;
			for (i = 0; i < <GenOverrideFunc>c__AnonStorey9D.list.get_Count(); i++)
			{
				int num = <GenOverrideFunc>c__AnonStorey9D.list.FindIndex((MethodInfo p) => ToLuaExport.CompareParmsCount(p, <GenOverrideFunc>c__AnonStorey9D.list.get_Item(i)));
				if (num >= 0 || (ToLuaExport.HasOptionalParam(<GenOverrideFunc>c__AnonStorey9D.list.get_Item(i).GetParameters()) && <GenOverrideFunc>c__AnonStorey9D.list.get_Item(i).GetParameters().Length > 1))
				{
					list.Add(<GenOverrideFunc>c__AnonStorey9D.list.get_Item(i));
				}
			}
			ToLuaExport.sb.AppendLine();
			MethodInfo methodInfo = <GenOverrideFunc>c__AnonStorey9D.list.get_Item(0);
			...
			int num6 = <GenOverrideFunc>c__AnonStorey9D.list.get_Item(1).GetParameters().Length + ((!<GenOverrideFunc>c__AnonStorey9D.list.get_Item(1).get_IsStatic()) ? 1 : 0);
			bool flag = true;
			...
			ToLuaExport.sb.AppendLine("\t\t{");
			int num7 = ToLuaExport.ProcessParams(methodInfo, 3, false, <GenOverrideFunc>c__AnonStorey9D.list.get_Count() > 1 && flag, beCheckTypes);
			ToLuaExport.sb.AppendFormat("\t\t\treturn {0};\r\n", num2 + num7);
			ToLuaExport.sb.AppendLine("\t\t}");
			for (int j = 1; j < <GenOverrideFunc>c__AnonStorey9D.list.get_Count(); j++)
			{
				flag = true;
				beCheckTypes = true;
				methodInfo = <GenOverrideFunc>c__AnonStorey9D.list.get_Item(j);
				...
				if (!ToLuaExport.HasOptionalParam(methodInfo.GetParameters()))
				{
					...
					if (list.Contains(<GenOverrideFunc>c__AnonStorey9D.list.get_Item(j)))
					{
		...
		private static void ProcessExtends(List<MethodInfo> list)
		{
			...
			if (ToLuaExport.extendType != null)
			{
				ToLuaExport.<ProcessExtends>c__AnonStorey9F <ProcessExtends>c__AnonStorey9F = new ToLuaExport.<ProcessExtends>c__AnonStorey9F();
				<ProcessExtends>c__AnonStorey9F.list2 = new List<MethodInfo>();
				<ProcessExtends>c__AnonStorey9F.list2.AddRange(ToLuaExport.extendType.GetMethods(4 | ToLuaExport.binding | 2));
				int i;
				for (i = <ProcessExtends>c__AnonStorey9F.list2.get_Count() - 1; i >= 0; i--)
				{
					if ((!<ProcessExtends>c__AnonStorey9F.list2.get_Item(i).get_Name().Contains("op_") && !<ProcessExtends>c__AnonStorey9F.list2.get_Item(i).get_Name().Contains("add_") && !<ProcessExtends>c__AnonStorey9F.list2.get_Item(i).get_Name().Contains("remove_")) || ToLuaExport.IsNeedOp(<ProcessExtends>c__AnonStorey9F.list2.get_Item(i).get_Name()))
					{
						list.RemoveAll((MethodInfo md) => md.get_Name() == <ProcessExtends>c__AnonStorey9F.list2.get_Item(i).get_Name());
						if (!ToLuaExport.IsObsolete(<ProcessExtends>c__AnonStorey9F.list2.get_Item(i)))
						{
							list.Add(<ProcessExtends>c__AnonStorey9F.list2.get_Item(i));
						}
					}
				}
			}
