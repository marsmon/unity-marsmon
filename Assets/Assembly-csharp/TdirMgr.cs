...
public class TdirMgr : MonoSingleton<TdirMgr>
{
	...
	{
		TdirMgr.<QueryTdirAsync>c__Iterator31 <QueryTdirAsync>c__Iterator = new TdirMgr.<QueryTdirAsync>c__Iterator31();
	...
	{
		TdirMgr.<QueryISP>c__Iterator32 <QueryISP>c__Iterator = new TdirMgr.<QueryISP>c__Iterator32();
			...
			for (int j = 1; j < array2.Length; j++)
			{
				if (array2[j].IndexOf('-') >= 0)
				{
					...
					if (array3.Length == 2)
					{
						...
						{
							int num2;
							if (int.TryParse(array3[1], ref num2))
							{
								if (num <= num2)
								{
									int num3 = num2 - num + 1;
	...
	public void StartGetHostAddress(TdirUrl tdirurl)
	{
		bool flag = false;
		IPAddress iPAddress;
		if (IPAddress.TryParse(tdirurl.addrs.get_Item(0).ip, out iPAddress))
		{
