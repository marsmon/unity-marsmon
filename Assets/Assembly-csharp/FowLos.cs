					...
					if (pFowMgr.m_pFieldObj.ViewBlockArrayImpl != null)
					{
						...
						while (enumerator.MoveNext())
						{
							...
							if ((int)viewBlockId != areaId || sViewBlockAttr.BlockType != 1)
							{
								...
								if (pFowMgr.m_pFieldObj.GrassBlockView || !pFowMgr.m_pFieldObj.ViewBlockAttrMap.TryGetValue((byte)areaId, ref sViewBlockAttr2) || sViewBlockAttr2.BlockType != 1)
								{
									if (this.ValidateViewBlock(current, cRaycastQuadrant.min.x, cRaycastQuadrant.max.x, cRaycastQuadrant.min.y, cRaycastQuadrant.max.y))
									{
										...
										while (enumerator2.MoveNext())
										{
											...
											BaseAlgorithm.AddUniqueItem<byte>(arg_2D4_0, current2.get_Key());
										}
										...
										while (enumerator2.MoveNext())
										{
											...
											BaseAlgorithm.AddUniqueItem<byte>(arg_310_0, current3.get_Key());
										}
									}
								}
							}
