	...
	public class LobbyLogic : Singleton<LobbyLogic>, IUpdateLogic
	{
		...
		public void ReqSingleGameFinish(bool ClickGameOver = false, bool bGmWin = false)
		{
			if (this.isLogin)
			{
				...
				{
					b = ((!Singleton<WinLose>.instance.LastSingleGameWin) ? 2 : 1);
				}
		...
		private void CreateBattleStatisticInfoData(HeroKDA HeroInfo, ref COMDT_HERO_BATTLE_STATISTIC_INFO stBattleStatisticInfo)
		{
			...
			stBattleStatisticInfo.wNoAIKillCnt = (ushort)HeroInfo.JudgeStat.KillNum;
			stBattleStatisticInfo.wNoAIDeadCnt = (ushort)HeroInfo.JudgeStat.DeadNum;
			stBattleStatisticInfo.wNoAIAssistCnt = (ushort)HeroInfo.JudgeStat.AssitNum;
			stBattleStatisticInfo.dwNoAICoinCnt = (uint)HeroInfo.JudgeStat.GainCoin;
			if (num > 0u)
			{
				stBattleStatisticInfo.wNoAITotalHurtRatio = (ushort)((long)(100 * HeroInfo.JudgeStat.HurtToAll) / (long)((ulong)num));
				stBattleStatisticInfo.bTotalHurtPercent = (byte)((long)(100 * HeroInfo.hurtToEnemy) / (long)((ulong)num));
			}
			if (num2 > 0u)
			{
				stBattleStatisticInfo.wNoAITotalHurtHeroRatio = (ushort)((long)(100 * HeroInfo.JudgeStat.HurtToHero) / (long)((ulong)num2));
			...
			if (num4 > 0u)
			{
				stBattleStatisticInfo.wNoAITotalBeHeroHurtRatio = (ushort)((long)(100 * HeroInfo.JudgeStat.SufferHero) / (long)((ulong)num4));
			}
			...
			{
				stBattleStatisticInfo.bEyeNum = ((HeroInfo.actorHero.handle.SkillControl.stSkillStat.m_uiSpawnEyeTimes >= 15u) ? 15 : ((byte)HeroInfo.actorHero.handle.SkillControl.stSkillStat.m_uiSpawnEyeTimes));
				...
				if (enumerator.Current.actorHero && enumerator.Current.actorHero.handle.ValueComponent != null)
				{
					...
					if (captain.handle.EquipComponent != null)
					{
						...
						while (enumerator2.MoveNext())
						{
							...
							arg_28E_0.dwEquipID = (uint)current.get_Key();
							...
							arg_2B9_0.dwFirstBuyTime = current2.get_Value();
							if (++num >= (int)b2)
							{
								break;
							}
		...
		private void CreateGeneralData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			if (num2 > 0)
			{
				...
				for (int l = 0; l < num3; l++)
				{
					...
					{
						cOMDT_GAME_30SEC_INFO.wNoAIKillCnt = (ushort)judgeStatc.KillNum;
						cOMDT_GAME_30SEC_INFO.wNoAIDeadCnt = (ushort)judgeStatc.DeadNum;
						cOMDT_GAME_30SEC_INFO.wNoAIAssistCnt = (ushort)judgeStatc.AssitNum;
						cOMDT_GAME_30SEC_INFO.wSelfBehaviorCoin = (ushort)judgeStatc.GainCoin;
						cOMDT_GAME_30SEC_INFO.dwHurtHeroCnt = (uint)judgeStatc.HurtToHero;
						cOMDT_GAME_30SEC_INFO.dwBeHurtHeroCnt = (uint)judgeStatc.SufferHero;
					}
					...
					{
						cOMDT_GAME_30SEC_INFO.wCampKillCnt = (ushort)campJudgeRecord.killNum;
						cOMDT_GAME_30SEC_INFO.wCampDeadCnt = (ushort)campJudgeRecord.deadNum;
						cOMDT_GAME_30SEC_INFO.dwTeamBehaviorCoin = (uint)campJudgeRecord.gainCoin;
						cOMDT_GAME_30SEC_INFO.dwTeamHurtHeroCnt = (uint)campJudgeRecord.hurtToHero;
						cOMDT_GAME_30SEC_INFO.dwTeamBeHurtHeroCnt = (uint)campJudgeRecord.sufferHero;
					}
		...
		private void FillVDStat(ref COMDT_SETTLE_GAME_GENERAL_INFO stGeneralData, uint playerID)
		{
			if (Singleton<BattleStatistic>.instance.m_vdStat != null)
			{
				...
				if (player != null)
				{
					...
					stGeneralData.bAdvantageNum = (((byte)Singleton<BattleStatistic>.instance.m_vdStat.count >= 16) ? 16 : ((byte)Singleton<BattleStatistic>.instance.m_vdStat.count));
					...
					stGeneralData.bSelfCampHaveWinningFlag = ((!Singleton<BattleStatistic>.instance.bSelfCampHaveWinningFlag) ? 0 : 1);
				}
			}
		}
