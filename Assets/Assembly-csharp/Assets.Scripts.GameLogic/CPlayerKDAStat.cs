...
namespace Assets.Scripts.GameLogic
{
	public class CPlayerKDAStat
	{
		...
		public int GetTeamHurtToHero(COM_PLAYERCAMP camp)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (current.get_Value().PlayerCamp == camp)
				{
					int arg_42_0 = num;
					KeyValuePair<uint, PlayerKDA> current2 = enumerator.Current;
					num = arg_42_0 + current2.get_Value().TotalHurtToHero;
				}
			}
			return num;
		}
		...
		public int GetTeamBeHurtByHero(COM_PLAYERCAMP camp)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (current.get_Value().PlayerCamp == camp)
				{
					int arg_42_0 = num;
					KeyValuePair<uint, PlayerKDA> current2 = enumerator.Current;
					num = arg_42_0 + current2.get_Value().TotalBeHurtByHero;
				}
			}
			return num;
		}
		...
		{
			cjr.killNum += heroKda.numKill;
			cjr.deadNum += heroKda.numDead;
			cjr.gainCoin += heroKda.TotalCoin - heroKda.CoinFromSystem;
			cjr.hurtToHero += heroKda.hurtToHero;
			cjr.sufferHero += heroKda.hurtTakenByHero;
		}
	}
}
