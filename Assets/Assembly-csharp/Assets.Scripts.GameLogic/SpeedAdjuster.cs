...
namespace Assets.Scripts.GameLogic
{
	public class SpeedAdjuster : Singleton<SpeedAdjuster>
	{
		...
		private int HandleSpeedUp(int baseValue, int totalValue, int orignalSpeed)
		{
			...
			{
				if (this.SpeedUpConfigs.get_Item(j).Intersect((!this.bReferenceBaseSpeedWhenSpeedUp) ? 0 : baseValue, totalValue))
				{
		...
		private int HandleSpeedDown(int baseValue, int totalValue, int orignalSpeed)
		{
			...
			{
				if (this.SpeedDownConfigs.get_Item(j).Intersect((!this.bReferenceBaseSpeedWhenSpeedUp) ? 0 : baseValue, totalValue))
				{
