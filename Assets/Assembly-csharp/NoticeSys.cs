...
public class NoticeSys : MonoSingleton<NoticeSys>
{
	...
	private bool CheckIsBtnUrl(string msgUrl, ref bool bTitle, ref string sTitle, ref NoticeSys.BTN_DOSOMTHING btnDoSth, ref string url)
	{
		...
		{
			string stringToUnescape = text4.Substring("title=".get_Length());
		...
		else if (text4.Contains("button=9"))
		{
			btnDoSth = NoticeSys.BTN_DOSOMTHING.BTN_DOSOMTHING_URL_APPEND;
		}
		string text5 = string.Empty;
		if (num2 - num3 - text3.get_Length() > 0)
		{
			text5 = msgUrl.Substring(num3 + text3.get_Length(), num2 - num3 - text3.get_Length());
		}
		if (bTitle)
		{
			url = text5;
		...
		if (text5.Contains("url="))
		{
			url = text5.Substring("url=".get_Length());
			return true;
		}
		url = string.Empty;
		btnDoSth = NoticeSys.BTN_DOSOMTHING.BTN_DOSOMTHING_NONE;
	...
	private void OnBtnComplete(CUIEvent ciEvent)
	{
		...
		else if (this.m_bGoto && this.m_bShow && this.m_btnUrl != string.Empty)
		{
			...
			if (btnDoSth == NoticeSys.BTN_DOSOMTHING.BTN_DOSOMTHING_URL_APPEND)
			{
				string text2 = text;
				if (!text2.Contains("?"))
				{
					text2 = text2 + "?" + CLobbySystem.GetAppendUrlParam();
				}
				else
				{
					text2 = text2 + "&" + CLobbySystem.GetAppendUrlParam();
				}
				Debug.Log("noticesys btn url " + text2);
				CUICommonSystem.OpenUrl(text2, true, 0);
			}
