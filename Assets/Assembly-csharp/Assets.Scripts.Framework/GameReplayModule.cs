			...
			if (watchEntry == null)
			{
				...
				else
				{
					for (int i = 0; i < beginLoadPkg.astCampInfo.Length; i++)
					{
						...
						{
							CSDT_CAMPPLAYERINFO cSDT_CAMPPLAYERINFO2 = cSDT_CAMPINFO.astCampPlayerInfo[(int)((UIntPtr)num3)];
				...
				for (int i = files.Length - 1; i >= 0; i--)
				{
					try
					{
						...
						if (text.EndsWith(".abc"))
						{
							...
							GameReplayModule.ReplayFileInfo replayFileInfo = new GameReplayModule.ReplayFileInfo();
							replayFileInfo.isExpired = !CVersion.IsSynchronizedVersion(text2, text3);
							replayFileInfo.path = text;
							replayFileInfo.heroId = binaryReader.ReadUInt32();
							replayFileInfo.startTime = binaryReader.ReadInt64();
							DebugHelper.Assert(replayFileInfo.startTime >= 0L && replayFileInfo.startTime < 3155378975999999999L, string.Format(" Error!! abc file -> appVer = {0} , resVer = {1} , heroId = {2} , startTime = {3}", new object[]
							{
								text2,
								text3,
								replayFileInfo.heroId,
								replayFileInfo.startTime
							}));
							replayFileInfo.mapType = binaryReader.ReadByte();
							replayFileInfo.mapId = binaryReader.ReadUInt32();
							replayFileInfo.userName = binaryReader.ReadString();
							replayFileInfo.userHead = Singleton<ApolloHelper>.GetInstance().ToSnsHeadUrl(binaryReader.ReadString());
							replayFileInfo.userRankGrade = binaryReader.ReadByte();
							replayFileInfo.userRankClass = binaryReader.ReadUInt32();
							listView.Add(replayFileInfo);
						}
						else
						{
							...
							GameReplayModule.ReplayFileInfo replayFileInfo2 = new GameReplayModule.ReplayFileInfo();
							replayFileInfo2.isExpired = !CVersion.IsSynchronizedVersion(versionString, versionString2);
							replayFileInfo2.path = text;
							replayFileInfo2.heroId = this.replayAbsHeader.dwHeroID;
							replayFileInfo2.startTime = (long)this.replayAbsHeader.ullTime;
							DebugHelper.Assert(replayFileInfo2.startTime >= 0L && replayFileInfo2.startTime < 3155378975999999999L, string.Format(" Error!! abs file -> aappVer = {0} , resVer = {1} , heroId = {2} , startTime = {3}", new object[]
							{
								versionString,
								versionString2,
								replayFileInfo2.heroId,
								replayFileInfo2.startTime
							}));
							replayFileInfo2.mapType = this.replayAbsHeader.bMapType;
							replayFileInfo2.mapId = this.replayAbsHeader.dwMapId;
							replayFileInfo2.userName = StringHelper.UTF8BytesToString(ref this.replayAbsHeader.szName);
							replayFileInfo2.userHead = StringHelper.UTF8BytesToString(ref this.replayAbsHeader.szHeadUrl);
							replayFileInfo2.userRankGrade = this.replayAbsHeader.bRankGrade;
							replayFileInfo2.userRankClass = this.replayAbsHeader.dwRankClass;
							listView.Add(replayFileInfo2);
						}
						binaryReader.Close();
						fileStream.Close();
						if (removeObsolete)
