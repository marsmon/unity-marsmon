		...
		public static void FakeLoadProcess(float progress)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current().Computer)
				{
					...
					GameObject gameObject = (enumerator.get_Current().PlayerCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? form.gameObject.transform.FindChild("DownPanel").FindChild(string.Format("Down_Player{0}", num)).gameObject : form.gameObject.transform.FindChild("UpPanel").FindChild(string.Format("Up_Player{0}", num)).gameObject;
					if (gameObject != null)
					{
