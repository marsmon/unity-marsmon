	...
	public class RankingSystem : Singleton<RankingSystem>
	{
		...
		protected void UpdateAllElementInView()
		{
			...
			else
			{
				...
				byte bLogicGrade = CLadderSystem.GetGradeDataByShowGrade((int)masterRoleInfo.m_rankGrade).bLogicGrade;
				uint rankScore = masterRoleInfo.m_rankScore;
				int num2 = 0;
				while ((long)num2 < (long)((ulong)num))
				{
					...
					if (this._curRankingType == RankingSystem.RankingType.Ladder)
					{
						...
						{
							b = CLadderSystem.GetGradeDataByShowGrade((int)this._sortedFriendRankList[num2].stRankShowGrade.bGrade).bLogicGrade;
							num3 = this._sortedFriendRankList[num2].stRankShowGrade.dwScore;
							num4 = this._sortedFriendRankList[num2].dwPvpLvl;
						}
						if (num2 < this._sortedFriendRankList.Count && bLogicGrade >= b && (bLogicGrade != b || rankScore >= num3) && (bLogicGrade != b || rankScore != num3 || masterRoleInfo.PvpLevel >= num4))
						{
							break;
						}
					}
					else
					{
						...
						if (num2 < this._sortedFriendRankList.Count)
						{
							num5 = this._sortedFriendRankList[num2].RankVal[(int)RankingSystem.ConvertRankingServerType(this._curRankingType)];
							uint dwPvpLvl = this._sortedFriendRankList[num2].dwPvpLvl;
						}
						if (num2 < this._sortedFriendRankList.Count && dwScore >= num5)
						{
							break;
						}
					}
			...
			{
				if (this._rankingList.GetElemenet(num6) != null && this._rankingList.IsElementInScrollArea(num6))
				{
					this.UpdateOneElement(this._rankingList.GetElemenet(num6).gameObject, num6);
				}
				num6++;
			}
			this._form.m_formWidgets[21].CustomSetActive(false);
