...
namespace Assets.Scripts.GameSystem
{
	public class CCustomRcmdEquipInfo
	{
		...
		public bool GetSelfDefineRcmdEquip(uint heroId, uint rcmdId, ref ushort[] equipIds)
		{
			...
			if (this.m_customRecommendEquipDictionary.TryGetValue(heroId, ref stRcmdEquipListInfo) && stRcmdEquipListInfo.ListItem[(int)((UIntPtr)rcmdId)].bSelfDefine)
			{
		...
		public void OnSelfDefineRcmdEquipChange(CSDT_HERO_EQUIPLIST svrEquipList)
		{
			...
			{
				stRcmdEquipListInfo.ListItem[(int)((UIntPtr)svrEquipList.dwEquipIndex)].bSelfDefine = true;
				for (int i = 0; i < 6; i++)
				{
					stRcmdEquipListInfo.ListItem[(int)((UIntPtr)svrEquipList.dwEquipIndex)].EquipId[i] = (ushort)svrEquipList.stEquipInfo.EquipID[i];
				}
		...
		public void OnRecoverSystemEquipRsp(SCPKG_RECOVER_SYSTEMEQUIP_RSP recoverDefaultRcmdEquip)
		{
			...
			{
				stRcmdEquipListInfo.ListItem[(int)((UIntPtr)recoverDefaultRcmdEquip.dwEquipIndex)].bSelfDefine = false;
				CCustomRcmdEquipInfo.SetDefaultRcmdEquip(ref stRcmdEquipListInfo.ListItem[(int)((UIntPtr)recoverDefaultRcmdEquip.dwEquipIndex)].EquipId, recoverDefaultRcmdEquip.dwHeroId, recoverDefaultRcmdEquip.dwEquipIndex);
