...
namespace Assets.Scripts.GameSystem
{
	...
	public class CGuildSystem : Singleton<CGuildSystem>
	{
		...
		{
			return Utility.IsSamePlatformWithSelf(info.stUin.dwLogicWorldId) && this.IsInNormalGuild() && CGuildSystem.HasManageAuthority() && info.bGuildState == 0 && info.dwPvpLvl >= CGuildHelper.GetGuildMemberMinPvpLevel();
		}
		...
		public static void ReceiveNewMemberNtf(CSPkg msg)
		{
			...
			if (!currentGuildInfo.listMemInfo.Contains(guildMemInfo))
			{
				...
				expr_26E_cp_0.briefInfo.bMemberNum = expr_26E_cp_0.briefInfo.bMemberNum + 1;
			}
			...
			else
			{
				...
				for (int i = listMemInfo.Count - 1; i >= 0; i--)
				{
					if (msg.stPkgData.stFireGuildMemberNtf.ullUid == listMemInfo[i].stBriefInfo.uulUid)
					{
						...
						expr_F1_cp_0.briefInfo.bMemberNum = expr_F1_cp_0.briefInfo.bMemberNum - 1;
						break;
					}
				}
