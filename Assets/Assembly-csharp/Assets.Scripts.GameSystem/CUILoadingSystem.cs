		...
		private static GameObject GetMemberItem(uint ObjId)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current().PlayerId == ObjId)
				{
					...
					return (enumerator.get_Current().PlayerCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? form.gameObject.transform.FindChild("DownPanel").FindChild(string.Format("Down_Player{0}", num)).gameObject : form.gameObject.transform.FindChild("UpPanel").FindChild(string.Format("Up_Player{0}", num)).gameObject;
				}
			}
			return null;
		}
