	...
	internal class CSettingsSys : Singleton<CSettingsSys>
	{
		...
		private void InitWidget(string formPath)
		{
			...
			else
			{
				...
				this.SetAntiDisturbUI(COM_OTHERSTATE_BIT_MASK.COM_OTHERSTATE_BIT_MASK_DND, ((Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().OtherStatebBits & 1u) != 0u) ? 1 : 0);
		...
		protected void OnSettingTabChange(CUIEvent uiEvent)
		{
			if (this._form != null && this._tabList != null)
			{
				...
				else if (settingType == CSettingsSys.SettingType.Operation)
				{
					...
					Singleton<CUINewFlagSystem>.GetInstance().DelNewFlag(this._form.m_formWidgets[78].transform.FindChild("Text").gameObject, enNewFlagKey.New_Setting_LastHitMode_V15, false);
				}
		...
		private void OnCancelAntiDisturbChange(CUIEvent uiEvent)
		{
			...
			int num = 1;
			this.SetAntiDisturbUI(COM_OTHERSTATE_BIT_MASK.COM_OTHERSTATE_BIT_MASK_DND, (((ulong)masterRoleInfo.OtherStatebBits & (ulong)((long)num)) != 0uL) ? 1 : 0);
		}
		...
		public void reqOperateUserPrivacyBit(bool bOpen, COM_USER_PRIVACY_MASK bit)
		{
			...
			cSPkg.stPkgData.stOperateUserPrivacyReq.bOpenOrClose = ((!bOpen) ? 0 : 1);
