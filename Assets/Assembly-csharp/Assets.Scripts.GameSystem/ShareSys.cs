...
namespace Assets.Scripts.GameSystem
{
	public class ShareSys : MonoSingleton<ShareSys>
	{
		...
		{
			ShareSys.<DownloadImageByTag2>c__Iterator2E <DownloadImageByTag2>c__Iterator2E = new ShareSys.<DownloadImageByTag2>c__Iterator2E();
			<DownloadImageByTag2>c__Iterator2E.preUrl = preUrl;
			<DownloadImageByTag2>c__Iterator2E.tagPath = tagPath;
			<DownloadImageByTag2>c__Iterator2E.req = req;
			<DownloadImageByTag2>c__Iterator2E.callBack = callBack;
			<DownloadImageByTag2>c__Iterator2E.<$>preUrl = preUrl;
			<DownloadImageByTag2>c__Iterator2E.<$>tagPath = tagPath;
			<DownloadImageByTag2>c__Iterator2E.<$>req = req;
			<DownloadImageByTag2>c__Iterator2E.<$>callBack = callBack;
			<DownloadImageByTag2>c__Iterator2E.<>f__this = this;
			return <DownloadImageByTag2>c__Iterator2E;
		}
		...
		{
			ShareSys.<Capture>c__Iterator2F <Capture>c__Iterator2F = new ShareSys.<Capture>c__Iterator2F();
			<Capture>c__Iterator2F.screenShotRect = screenShotRect;
			<Capture>c__Iterator2F.callback = callback;
			<Capture>c__Iterator2F.<$>screenShotRect = screenShotRect;
			<Capture>c__Iterator2F.<$>callback = callback;
			<Capture>c__Iterator2F.<>f__this = this;
			return <Capture>c__Iterator2F;
		}
		...
		private bool UnPackSNSDataStr(string data)
		{
			...
			if (text2 != null)
			{
				if (ShareSys.<>f__switch$map3 == null)
				{
					...
					ShareSys.<>f__switch$map3 = dictionary;
				}
				int num;
				if (ShareSys.<>f__switch$map3.TryGetValue(text2, ref num))
				{
					switch (num)
					{
					case 0:
		...
		{
			Debug.Log("QQGameTeamCreate " + roomInfoStr);
			...
			if (num == 3 || num == 4)
			{
				if (cOM_ROOM_TYPE == COM_ROOM_TYPE.COM_ROOM_TYPE_MATCH)
				{
					if (Singleton<LobbyLogic>.instance.isLogin)
					{
						...
						if (num == 4)
						{
							...
							Debug.Log("QQGameTeamCreate sendmsg1");
						}
						else
						{
							...
							Debug.Log("QQGameTeamCreate sendmsg2");
						}
					}
					return;
				}
				if (cOM_ROOM_TYPE == COM_ROOM_TYPE.COM_ROOM_TYPE_NORMAL)
				{
					if (Singleton<LobbyLogic>.instance.isLogin)
					{
						...
						Debug.Log("QQGameTeamCreate sendmsg3");
					}
					return;
				}
			}
			else if (num == 8)
			{
				if (Singleton<LobbyLogic>.instance.isLogin)
				{
					...
					if (int.TryParse(array[3], ref cOMDT_QQSPROT_EXTRA.iRoomEntity) && uint.TryParse(array[4], ref cOMDT_QQSPROT_EXTRA.dwRoomID) && byte.TryParse(array[5], ref cOMDT_QQSPROT_EXTRA.bAddType) && uint.TryParse(array[6], ref cOMDT_QQSPROT_EXTRA.dwTimeOutSec))
					{
						...
						Debug.Log("QQGameTeamCreate sendmsg4");
					}
				}
				return;
			}
		...
		{
			ShareSys.<QQGameTeamStateChgGet>c__Iterator30 <QQGameTeamStateChgGet>c__Iterator = new ShareSys.<QQGameTeamStateChgGet>c__Iterator30();
			<QQGameTeamStateChgGet>c__Iterator.url = url;
			<QQGameTeamStateChgGet>c__Iterator.<$>url = url;
			return <QQGameTeamStateChgGet>c__Iterator;
		}
		...
		private bool CheckQQGameTeamInfo(COM_ROOM_TYPE roomType, byte mapType, uint mapID)
		{
			...
			return array2 != null && (array2.Length == 3 || array2.Length == 8 || array2.Length == 4) && Convert.ToUInt32(array2[0]) == (uint)roomType && Convert.ToByte(array2[1]) == mapType && Convert.ToUInt32(array2[2]) == mapID;
		}
