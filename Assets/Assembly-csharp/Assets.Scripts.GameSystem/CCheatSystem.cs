...
namespace Assets.Scripts.GameSystem
{
	public class CCheatSystem : Singleton<CCheatSystem>
	{
		...
		private static string s_rmsUse60FPS = "RMS_Use60FPS";

		private int m_originalFPS = GameFramework.c_renderFPS;

		private bool m_isInOpening;
		...
		public override void Init()
		{
			...
			CCheatSystem.s_joystickConfigs[2] = (PlayerPrefs.GetInt(CCheatSystem.s_rmsUse60FPS, 0) == 1);
			if (CCheatSystem.s_joystickConfigs[2])
			{
				this.UpdateFPS();
			}
		...
		private void OnCheatJoystickConfigChanged(CUIEvent uiEvent)
		{
			...
			{
				bool flag2 = CCheatSystem.s_joystickConfigs[i] != multiSelected[i];
				if (flag2)
				{
					...
					{
						PlayerPrefs.SetInt(CCheatSystem.s_rmsUse60FPS, (!CCheatSystem.s_joystickConfigs[i]) ? 0 : 1);
						flag = true;
						this.UpdateFPS();
					}
				}
