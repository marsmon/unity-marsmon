	...
	public class CRoleInfo
	{
			...
			if (this.bNewbieAchieveChanged || resetOldPlayerGuided)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					for (int j = 0; j < 64; j++)
					{
						...
						{
							cSPkg.stPkgData.stUpdateClientBitsNtf.stClientBits.BitsDetail[i] += ((!this.NewbieAchieveBits.get_Item(64 * i + j)) ? 0uL : (1uL << j));
						}
					}
				}
		...
		public void SyncClientBitsToSvr()
		{
			if (this._clientBitsChanged)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					for (int j = 0; j < 64; j++)
					{
						cSPkg.stPkgData.stUpdNewClientBits.stClientBits.BitsDetail[i] += ((!this._clientBits.get_Item(64 * i + j)) ? 0uL : (1uL << j));
					}
				}
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		...
		public void ReqSetInBattleNewbieBit(uint id, bool bIsLastAge, int time = 0)
		{
			...
			if (curLvelContext != null && curLvelContext.IsGameTypeGuide())
			{
				...
				cSPkg.stPkgData.stDyeInBattleNewbieBitReq.bIsLastAge = ((!bIsLastAge) ? 0 : 1);
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
