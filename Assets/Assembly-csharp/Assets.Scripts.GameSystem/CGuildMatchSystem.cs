		...
		private void RefreshInvitationList()
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (guildMemberInfoByUid != null)
				{
					...
					component7.text = this.GetInvitationStateText((COM_GUILDMATCH_INVITECARD_STATE)this.m_invitationInfos.get_Item(i).invitationInfo.bState, this.m_invitationInfos.get_Item(i).isValid);
			...
			if (this.m_signUpListForm != null)
			{
				...
				for (int i = 0; i < component.GetElementAmount(); i++)
				{
					...
					if (elemenet != null)
					{
						...
						ulong num;
						if (ulong.TryParse(component2.text, ref num) && num == targetUid)
						{
			...
			if (this.m_signUpInfos != null)
			{
				for (int i = 0; i < this.m_signUpInfos.get_Count(); i++)
				{
					if (this.m_signUpInfos.get_Item(i).uid == Singleton<CRoleInfoManager>.GetInstance().masterUUID)
					{
						...
						stSignUpInfo.memo = ((this.m_signUpMemoText == null) ? string.Empty : this.m_signUpMemoText);
