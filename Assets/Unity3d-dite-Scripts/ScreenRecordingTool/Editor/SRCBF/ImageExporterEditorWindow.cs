...
namespace taecg.tools.ImageExporter
{
    ...
    public class ImageExporterEditorWindow : EditorWindow
    {
        ...
        [MenuItem("Dite_Tools/Image Exporter(序列图导出)")]
        ...
        {           
            ...
            //     EditorGUILayout.LabelField(NAME_currectFrame, oo.ToString(), style);
            // }
                ...
                switch (theImageFormat)
                {
                    ...
                    case ImageFormat.PNG:
                        ...
                        if (isEnabledAlpha)
                        {
                            ...
                            // Color ncol = cam.backgroundColor;
                            this.bgcolor.a = 0;
            ...
            GUILayout.Space(10);
            // EditorGUILayout.LabelField("输出地址类：");
            {
                EditorGUILayout.BeginHorizontal();
                GUIStyle style = new GUIStyle("textfield");
                EditorGUILayout.LabelField(NAME_setFileName,GUILayout.Width(120),GUILayout.ExpandWidth(false));
                fileName = EditorGUILayout.TextField(fileName, style);
                
                this.Isaddtime = EditorGUILayout.Toggle(this.Isaddtime,GUILayout.Width(15),GUILayout.ExpandWidth(false));
                EditorGUILayout.EndHorizontal();
            }
            
            {
                ...
                EditorGUILayout.LabelField(NAME_setSavePath,GUILayout.Width(120),GUILayout.ExpandWidth(false));
            ...
            if(A)
            {
                //导出当前截图
                {
                    // EditorGUILayout.LabelField("单帧导出：");
                    // GUILayout.Space(20); dite
                    EditorGUILayout.BeginHorizontal();
                    // if (GUILayout.Button("预览", GUILayout.Height(30)))
                    // {
                    //     int w = Mathf.Max(Mathf.Min((int)resolution.x+20,1024+20),256+20) ;
                    //     int h = Mathf.Max(Mathf.Min((int)resolution.x+20,1024+20),256+20) ;
                    //     // MyWindowdite window00 = (MyWindowdite)EditorWindow.GetWindowWithRect(
                    //     //     typeof(MyWindowdite),
                    //     //     new Rect(0,0,w,h+35)
                    //     // );
                    //     ImageExporterEditorChildrenWindowdite window00 = (ImageExporterEditorChildrenWindowdite)EditorWindow.GetWindow(typeof(ImageExporterEditorChildrenWindowdite));
                    //     window00.titleContent = new GUIContent("Render View");
                    
                    //     window00.position = new Rect(this.position.x+this.position.width,this.position.y,w,h);
                    //     // window00.autoRepaintOnSceneChange = true;
                    //     window00.Show();
                    //     window00.m_Texture2D = TakeScreenShotView();
                    //     window00.isalp = this.isEnabledAlpha;
                    //     window00.inwidth = w -20 ;//(int)resolution.x;
                    //     window00.inhight = h -20;//(int)resolution.y;
                    // }

                    if (GUILayout.Button(NAME_takeScreenShot, GUILayout.Height(30)))
                    {
                        this.bgcolor.a = 0;
                        cam.backgroundColor = this.bgcolor;
                        TakeScreenShot();
            ...
            if(B)
            {
                //导出序列图
                {
                    // EditorGUILayout.LabelField("序列导出：");
                    ...
                    // if(B)
                    {
                        ...
                        // Rect rc0 = EditorGUILayout.GetControlRect();
                        EditorGUILayout.BeginVertical(GUILayout.Height(this.jiange_dite),GUILayout.ExpandWidth(true));
        ...
        //dite
        Texture2D TakeScreenShotView(){
            int resWidthN = (int)resolution.x;
            int resHeightN = (int)resolution.y;
            this.rendertex = new RenderTexture(resWidthN, resHeightN, 24);
            cam.targetTexture = this.rendertex;
        ...
        void TakeScreenShot()
        {
            ...
            if(Isaddtime)
            {
                _outfileName = fileName + "_" + System.DateTime.Now.ToString("yyyy-MM-dd_HH-mm-ss");
            }
