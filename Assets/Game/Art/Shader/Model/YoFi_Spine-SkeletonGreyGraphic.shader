...
Shader "YoFi/Spine/SkeletonGreyGraphic"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			inline float4 blendmodel(in float4 finalcol) {
				float maxColor = max(finalcol.r, max(finalcol.g, finalcol.b));
				float minColor = min(finalcol.r, min(finalcol.g, finalcol.b));
				float lightness = (maxColor + minColor) / 2.0;
				return fixed4(lightness, lightness, lightness, finalcol.a);
			}
