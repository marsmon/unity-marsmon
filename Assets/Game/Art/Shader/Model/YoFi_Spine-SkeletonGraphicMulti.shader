...
Shader "YoFi/Spine/SkeletonGraphicMulti"
{
	Properties
	{
		
		[Header(YoFi Custom)]
		// [NoScaleOffset] _FristTex("Frist Texture2", 2D) = "black" {}
		[NoScaleOffset] _SecondTex("Second Texture", 2D) = "black" {}
		[NoScaleOffset] _ThirdTex("Third Texture", 2D) = "black" {}
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _MainTex;
			sampler2D _SecondTex,_ThirdTex; //_FristTex,

			fixed4 frag (VertexOutput IN) : SV_Target
			{
				half4 texColor = 0;// tex2D(_MainTex, IN.texcoord);

				float id = IN.color.g;
				if(id > 0.9){
					texColor = tex2D(_MainTex, IN.texcoord);
				}else if(id < 0.1){
					texColor = tex2D(_ThirdTex, IN.texcoord);
				}else{
					texColor = tex2D(_SecondTex, IN.texcoord);
				}
				...
				half4 color = (texColor + _TextureSampleAdd) ;//这里我们顶点色被用做其他作用了所以这里不能赋值 * IN.color;
