Shader "YoFi/Spine/SkeletonMulti" {
	Properties{
		[Header(Custom)]
	...
	SubShader{
		...
		Pass {
			...
			float4 frag(VertexOutput i) : SV_Target{
				float4 finalcol = 0;
				float4 texColor = 0;
				float id = i.vertexColor.g;
				if(id > 0.9){
					texColor = tex2D(_MainTex, i.uv.xy);
				}else if(id < 0.1){
					texColor = tex2D(_ThirdTex, i.uv.xy);
				}else{
					texColor = tex2D(_SecondTex, i.uv.xy);
				}
				...
				texColor.a = i.vertexColor.a * texColor.a;
				finalcol = texColor;
				// 获取特效使用的全覆盖颜色
